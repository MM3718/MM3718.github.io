...
def get_wordlevel_reprs(speechreps, word_align, align_type):
    """
    ...
    """
    if align_type == "mfa":
        # word_align = {'wordtype': 'the', 'utt_id': 'LJ003-0001', 'example_no': 1, 'start': 0.0, 'end': 0.06, 'utt_dur': 9.43442}
        ...
        # print("get_word_level speechreps:", speechreps)
        # print("word align:", word_align)
        return speechreps[start_idx:end_idx]

    elif align_type == "ctc":
        # word_align = {'wordtype': 'the', 'utt_id': 'LJ003-0001', 'example_no': 1, 'start_frame_idx': 1, 'end_frame_idx': 25}
        return speechreps[word_align["start_frame_idx"]-1:word_align["end_frame_idx"]] # speechreps[0:3]

def get_word2speechreps(ids, ids2speechreps, ids2word_alignments, align_type):
    """
    building a dictionary for all of the words in the corpus.
    key is the word and the value is a list of speech reps
    for example, if the word "hello" occurs ten times in the
    corpus then we have ten different sequences of speech reps

    Args:
        ids: all of the utterances
        ids2speechreps: {
        'LJ018-0162' : [71 71 67 45 34 56 85]
        'LJ041-0140' : [71 86 34 34 98 65 32]
        }
        ids2word_alignments:{
        'LJ003-001': [{'wordtype' : 'the'
                       'uttid' : 'LJ003-0001'
                       'example_no' : 1
                       'start' : 0.0
                       'end' : 0.06
                       'utt_dur' 9.43442}]
        'LJ003-002' : [{'wordtype' : 'the'
                       ...
                       {]
        }
        utt_id: a single utterance

    Returns: word2speechreps
        'might': {'LJ003-0010|1': [16, 50, 50, 50, 24, 13, 58, 58, 32, 65, 74, 2, 2],
                  'LJ003-0012|1': [60, 50, 50, 24, 13, 13, 58, 58, 32, 74, 2, 2, 2],
                  'LJ003-0090|1': [93, 93, 50, 24, 13, 58, 58, 58, 74, 74, 2, 2]},
        'the': {'LJ003-0001|1': [71, 73, 73],
                'LJ003-0002|1': [71, 76, 73, 62],
                'LJ003-0002|2': [82, 82, 62, 1]
                ...}
    """
    # print("in get_word2speechreps!!!!", ids2speechreps.keys())
    # print("ids:", ids)
    # print("ids2speechreps:", ids2speechreps.keys())
    # print("ids2word_alignments:", ids2word_alignments)

    word2speechreps = {}
    for utt_id in ids:  # utt_id = 'LJ003-001'
        speech_reps = ids2speechreps[utt_id]  # [71 71 67 45 34 56 85]
        word_aligns = ids2word_alignments[utt_id]  # [{'wordtpye' : 'the', 'uttid' : 'LJ003-0001', 'example_no' : 1, 'start' : 0.0, 'end' : 0.06', utt_dur' 9.43442}]

        # print("word_aligns!!!", word_align)

        for word_align in word_aligns:
            word_align['speech_reps'] = get_wordlevel_reprs(speech_reps, word_align, align_type)

            # print("word_aligns_speech_reps!!", word_align)
            # word_aligns_speech_reps!! {'wordtype': 'the', 'utt_id': 'LJ003-0001', 'example_no': 1, 'start': 0.0,
            #                            'end': 0.06, 'utt_dur': 9.43442, 'speech_reps': [71, 73, 73]}
    ...
    # print("word2speechreps!!!:", word2speechreps)
    # word2speechreps: {'the': {'LJ003-0001|1': [71, 73, 73], 'LJ003-0002|1': [71, 76, 73, 62], 'LJ003-0002|2': [82, 82, 62, 1]...},
    # {'was': {{'LJ003-0002|1': [57, 14, 76, 76, 29, 29, 28, 28, 23], 'LJ003-0004|1': [60, 14, 76, 76, 29, 28, 28], }}

    return word2speechreps


def get_wordlevel_reps_ctc(speechreps, word_align):
    """
    extract subsequence of 'repr' that corresponds to a particular word
    function expects input to be of dimension 2: (frames, hidden_size)
    """
    pass


def get_word2speechreps_ctc():
    pass
...
def get_speechreps_for_word(wordtype, utt_id, count_of_word, word2speechreps,
    ...
    get speechreps for one word, whereas
    `get_speechreps_inputs` in line 485
    get speechreps for whole utterance
    ...
    if random.random() > 1.0 - randomise_examples_p:
        ...
        # print("word2speechreps_ctc!!!", word2speechreps)
