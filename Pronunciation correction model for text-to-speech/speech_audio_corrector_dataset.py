...
class SpeechAudioCorrectorDataset(TextToSpeechDataset):
    def __init__(
        ...
        print("***  self.align_type:", self.align_type)
...
class SpeechAudioCorrectorDatasetCreator(TextToSpeechDatasetCreator):
    ...
    def _from_list(
        ...
        cls.word2speechreps_dir = "/Users/margaretlin/Mt/UoE/Dissertation/data/word2speechreps"

        alignment_type = args.align_type  # TODO change me!!!


        # LJSpeech word2speechreps
        if alignment_type == "mfa":
            speechrep_file = args.quantized_speechreps_file # "/Users/margaretlin/Mt/UoE/Dissertation/fairseq/examples/speech_audio_corrector/lj_speech_quantized.txt"
            num_clusters = get_num_clusters(speechrep_file)
            alignments_dir = "/Users/margaretlin/Mt/UoE/Dissertation/data/ljspeech_MFA_alignments_from_fb"
            word2speechreps, ids2word_alignments = cls.get_word2speechreps(
                speechrep_file, alignments_dir, ids=ids, corpus="ljspeech",
                split=split_name, align_type=args.align_type, force_creation=args.recreate_word2speechreps,
                num_clusters=num_clusters,
            )
            print(f"Align_codes: Successfully create, {split_name}, mfa word2speechreps")

        elif alignment_type == "ctc":
            # load e2e alignments from disk
            speechrep_file = args.quantized_speechreps_file  # "/Users/margaretlin/Mt/UoE/Dissertation/fairseq/examples/speech_audio_corrector/lj_speech_quantized.txt"
            num_clusters = get_num_clusters(speechrep_file)
            alignments_dir = "/Users/margaretlin/Mt/UoE/Dissertation/data/LJSpeech/hubert_CTC_word_level_reps"
            word2speechreps, ids2word_alignments = cls.get_word2speechreps(
                speechrep_file, alignments_dir, ids=ids, corpus="ljspeech",
                split=split_name, align_type=args.align_type, force_creation=args.recreate_word2speechreps,
                num_clusters=num_clusters,
            )
            print(f"Align_codes: Successfully create, {split_name}, ctc word2speechreps")
        ...
        # ext_speechrep_file = "/home/s1785140/fairseq/examples/speech_audio_corrector/vctk_fairseq_quantized_km100.txt"
    ...
    @classmethod
    def load_word_alignments(cls, ids, alignments_dir, align_type):
        if align_type == "mfa":
            ...
            # print("utt_id2word_alignment:", utt_id2word_alignments)
            # {'LJ003-0001':[{'wordtype': 'the', 'utt_id': 'LJ003-0001', 'example_no': 1, 'start':0.0, 'end':0.006, 'utt_dur': 9.43442}, {'wordtype': 'chronicle', ...}]}
            return utt_id2word_alignments
        elif align_type == "ctc":
            utt_id2word_alignments = {}
            for utt_id in ids:
                # alignments_dir = "/Users/margaretlin/Mt/UoE/Dissertation/data/LJSpeech/hubert_CTC_word_level_reps"
                words_align = cls.get_word_alignments_ctc(
                    pickle_path=f"{alignments_dir}/{utt_id}.pickle",
                    )
                utt_id2word_alignments[utt_id] = words_align
            return utt_id2word_alignments

    @classmethod
    def get_word2speechreps(cls, speechrep_file, alignments_dir, ids, corpus, split, align_type,
        ...
        filename = f"{prepend_str}_km{num_clusters}_{align_type}_word2speechreps.pickle"
        ...
        if os.path.isfile(filepath) and not force_creation:
            ...
            # print("ids2speechreps!!!", ids2speechreps)
            ...
            ids2word_alignments = cls.load_word_alignments(ids, alignments_dir, align_type)
            word2speechreps = get_word2speechreps(ids, ids2speechreps, ids2word_alignments, align_type)
            # save dict
    ...
    def get_word_alignments(
        ...
        # print("textgrid_path:", textgrid_path)
        ...
        for word in words_intervaltier:
            ...
            # print("words_intervaltier:", words_intervaltier)  # <words_intervaltier: IntervalTier words, 27 intervals>
            # print("word.mark:", word.mark)  # word.mark: chronicles
            # print("word:", word)  # word: Interval(0.0, 0.06, the)
    ...
    @classmethod
    def get_word_alignments_ctc(cls, pickle_path):
        # pickle_path = /Users/margaretlin/Mt/UoE/Dissertation/data/LJSpeech/hubert_CTC_word_level_reps/LJ001-0001.pickle
        file = open(pickle_path, 'rb')
        ctc_wd_level_rep = pickle.load(file)# [('HELLO', (1, 25)), ('WE', (51, 56)), ('ARE', (120, 126)), ...]
        file.close()
        words = []
        counter = Counter()

        for item in ctc_wd_level_rep:
            if item[0]:
                counter[item[0]] += 1
                words.append({
                    "wordtype": item[0].lower(),  # 'hello'
                    "utt_id": pickle_path.split('/')[-1].split('.')[0],  # 'LJ001-0001'
                    "example_no": counter[item[0]],  # 'example_no': 1
                    "start_frame_idx": item[1][0],  # 'start_frame_idx': 1
                    "end_frame_idx": item[1][1],  # 'end_frame_idx': 25
                })
        return words
