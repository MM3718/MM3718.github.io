...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, state, encoder_padding_mask):
        ...
        # print('state: ', state.size(), 'encoder_padding_mask: ', (encoder_padding_mask.size() if encoder_padding_mask is not None else None))
        # state.size() = [src_time_steps, batch_size, embed_dim]
        # encoder_padding_mask.size() = [batch_size, src_time_steps]
        # encoder_padding_mask is to zero-out the paddings added onto the input with shorted sequences
        # The output shape of `state' tensor will be [src_time_steps, batch_size, embed_dim]
        '''
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self,
        ...
        # print('state: ', state.size(), 'encoder_out: ', encoder_out.size(), 'encoder_padding_mask: ', encoder_padding_mask.size() if encoder_padding_mask else None, 'attn: ', attn.size() if attn is not None else None)
        # (input) state.size() = [tgt_time_steps, batch_size, embed_dim]
        # encoder_out.size() = [src_time_steps, batch_size, embed_dim]
        # encoder_padding_mask = [batch_size, src_time_steps]
        # (output) state.size() = [tgt_time_steps, batch_size, embed_dim]
        # attn.size() = [num_heads, batch_size, tgt_time_steps, src_time_steps]
        # encoder attention is not masked while self attention is masked
        # key_padding_mask is to zero out the padding added to shorter sequence, while attn_mask is to zero out the future timesteps to prevent the model to be able to see them and learn to copy them in training
        # there is no output directly as input into this layer
        '''
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self,
        ...
        q = self.q_proj(query) # [tgt_time_steps, batch_size, embed_dim]
        k = self.k_proj(key) # [src_time_steps, batch_size, embed_dim]
        v = self.v_proj(value) # [src_time_steps, batch_size, embed_dim]

        # computing scaled dot-product attention for h attention heads
        q = q.transpose(0, 1) # [batch_size, tgt_time_steps, embed_dim]
        k = k.transpose(0, 1) # [batch_size, src_time_steps, embed_dim]
        v = v.transpose(0, 1) # [batch_size, src_time_steps, embed_dim]

        q = q.view(batch_size, tgt_time_steps, self.num_heads, self.head_embed_size) # [batch_size, tgt_time_steps, num_heads, embed_dim / num_heads]
        k = k.view(batch_size, src_time_steps, self.num_heads, self.head_embed_size) # [batch_size, src_time_steps, num_heads, embed_dim / num_heads]
        v = v.view(batch_size, src_time_steps, self.num_heads, self.head_embed_size) # [batch_size, src_time_steps, num_heads, embed_dim / num_heads]

        q = q.transpose(1, 2) # [batch_size, num_heads, tgt_time_steps, embed_dim / num_heads]
        k = k.transpose(1, 2) # [batch_size, num_heads, src_time_steps, embed_dim / num_heads]
        v = v.transpose(1, 2) # [batch_size, num_heads, src_time_steps, embed_dim / num_heads]

        q = q.view(batch_size * self.num_heads, tgt_time_steps, self.head_embed_size) # [batch_size * num_heads, tgt_time_steps, embed_dim / num_heads]
        k = k.view(batch_size * self.num_heads, src_time_steps, self.head_embed_size) # [batch_size * num_heads, src_time_steps, embed_dim / num_heads]
        v = v.view(batch_size * self.num_heads, src_time_steps, self.head_embed_size) # [batch_size * num_heads, src_time_steps, embed_dim / num_heads]

        qk = torch.bmm(q, k.transpose(1, 2)) # [batch_size * num_heads, tgt_time_steps, src_time_steps]
        qk = qk / self.head_scaling
        if key_padding_mask is not None:
            qk.masked_fill_(key_padding_mask.repeat(self.num_heads, 1).unsqueeze(1), float('-inf'))
        if attn_mask is not None:
            mask = attn_mask.eq(float('-inf')) # [tgt_time_steps, tgt_time_steps]
            qk.masked_fill_(mask.unsqueeze(0), float('-inf'))
        qk = F.softmax(qk, dim=2)
        attentions = torch.bmm(qk, v) # [batch_size * num_heads, tgt_time_steps, embed_dim / num_heads]

        # concatenation of heads and output projection
        attentions = attentions.view(batch_size, self.num_heads, tgt_time_steps, self.head_embed_size) # [batch_size, num_heads, tgt_time_steps, embed_dim / num_heads]
        attentions = attentions.transpose(1, 2).contiguous() # [batch_size, tgt_time_steps, num_heads, embed_dim / num_heads]
        attentions = attentions.view(batch_size, tgt_time_steps, embed_dim) # [batch_size, tgt_time_steps, embed_dim]
        attentions = attentions.transpose(0, 1) # [tgt_time_steps, batch_size, embed_dim]
        attn = self.out_proj(attentions) # [tgt_time_steps, batch_size, embed_dim]

        attn_weights = qk.view(self.num_heads, batch_size, tgt_time_steps, src_time_steps) if need_weights else None
        # TODO: --------------------------------------------------------------------- CUT
