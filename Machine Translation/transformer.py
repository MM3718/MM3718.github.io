...
class TransformerEncoder(Seq2SeqEncoder):
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        # src_tokens.shape = [batch_size, src_time_steps]
        # Embed tokens indices
        embeddings = self.embed_scale * self.embedding(src_tokens)
        # embeddings.shape = [batch_size, src_time_steps, embed_dim]
        ...
        # embeddings.size() = [batch_size, src_time_steps, embed_dim]
        # It is for the transformer model to be aware of the position of the input token
        # otherwise it would be a set of input and the order of the input is not used in the model
        # print('src_tokens: ', src_tokens.shape, 'embeddings: ', embeddings.shape)
        '''
...
class TransformerDecoder(Seq2SeqDecoder):
    ...
    def forward(self, tgt_inputs, encoder_out=None, incremental_state=None, features_only=False):
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            # print('forward_state: ', forward_state.shape, 'self_attn_mask: ', self_attn_mask.shape)
            # It is to prevent the transformer from learning copying the input from the next time step
            # Decoder uses masked training which has already been masked
            # For incremental decoding, decoder does not have access to the next time step
            '''
        ...
        if not features_only:
            ...
            '''
            # forward_state.size() = [batch_size, tgt_time_steps, decoder_embed_dim]
            forward_state = self.embed_out(forward_state)
            # print('forward_state: ', forward_state.size())
            # forward_state.size() = [batch_size, tgt_time_steps, dictionary length]
            # Linear projection to map the hidden state onto output space, which is for each possible output tokens, later use softmax for probability for the next token
            # The output would represent features of the decoder output, can be used for other tasks e.g. binary classification
            '''
