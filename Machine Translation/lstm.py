...
class LSTMEncoder(Seq2SeqEncoder):
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        # final_hidden_states = [2 * num_layers, batch_size, hidden_size]
        # final_cell_states = [2 * num_layers, batch_size, hidden_size]
        ...
        if self.bidirectional:
            ...
            # final_hidden_state.size = [2 * num_layers, batch_size, hidden_size]
            final_hidden_states = combine_directions(final_hidden_states)
            # final_hidden_states = [num_layers, batch_size, 2 * hidden_size]
            final_cell_states = combine_directions(final_cell_states)
            # final_cell_states = [num_layers, batch_size, 2 * hidden_size]
...
class AttentionLayer(nn.Module):
    ...
    def forward(self, tgt_input, encoder_out, src_mask):
        ...
        # print(tgt_input.shape, encoder_out.shape, )
        ...
        # attn_weights[0].size = [1, src_time_steps]
        # encoder_out[0].size = [src_time_steps, output_dims]
...
class LSTMDecoder(Seq2SeqDecoder):
    ...
    def __init__(self,
        ...
        if self.use_lexical_model:
            ...
            # TODO: --------------------------------------------------------------------- CUT
            self.lexical_ffnn_1 = nn.Linear(embed_dim, embed_dim, bias=False)
            self.lexical_ffnn_2 = nn.Linear(embed_dim, len(dictionary))
            # TODO: --------------------------------------------------------------------- /CUT
    ...
    def forward(self, tgt_inputs, encoder_out, incremental_state=None):
        ...
        for j in range(tgt_time_steps):
            ...
            if self.attention is None:
                ...
                # tgt_hidden_states[-1].size = [batch_size, hidden_size]
                # src_out.size = ([src_time_steps, batch_size, output_dims], [num_layers, batch_size, [2] * hidden_size], [num_layers, batch_size, [2] * hidden_size])
                ...
                if self.use_lexical_model:
                    ...
                    # print('step_attn_weights: ', step_attn_weights.shape)
                    lexical_context = torch.tanh(torch.sum(step_attn_weights.transpose(0, 1).unsqueeze(2) * src_embeddings, dim=0))
        ...
        if self.use_lexical_model:
            ...
            lexical_contexts_tensor = torch.zeros((batch_size, tgt_time_steps, self.embed_dim))
            for i, lexical_context in enumerate(lexical_contexts):
                # print(lexical_contexts_tensor.size(), lexical_context.size(), decoder_output.size())
                lexical_contexts_tensor[:, i, :] = lexical_context
            h = torch.tanh(self.lexical_ffnn_1(lexical_contexts_tensor)) + lexical_contexts_tensor
            decoder_output = decoder_output + self.lexical_ffnn_2(h)
            # TODO: --------------------------------------------------------------------- /CUT
